...
app.Configure(config =>
{
    ...
    config
        ...
        .WithAlias("ver");
    config.AddCommand<GetTypeTestCommand>("test").IsHidden();
    config.AddCommand<OnlyStreamLoadCommand>("pestream").IsHidden();

    config.AddCommand<LoadCommand>("load").WithDescription(CliLocalizationString.cmd_load_desc);

    config.AddCommand<SaveCommand>("save").WithDescription(CliLocalizationString.cmd_save_desc);
    config
        .AddCommand<UnbindFatherCommand>("unbind-father")
        .WithAlias("unbind")
        .WithDescription(CliLocalizationString.cmd_rpe_unbind_father_desc);
    config
        .AddCommand<FitEventCommand>("fit")
        .WithAlias("fit-event")
    ...
    config.AddBranch(
        ...
        ws =>
        {
            ...
            ws.AddCommand<LoadCommand>("load").WithDescription(CliLocalizationString.cmd_load_desc);
            ws.AddCommand<SaveCommand>("save").WithDescription(CliLocalizationString.cmd_save_desc);
        }
    );
});
